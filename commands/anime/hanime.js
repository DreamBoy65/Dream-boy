...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!query){
      return message.error('\\Please include your **hanime** query!');
    };

    const res = await hanime.search(query);

    if (!res.hits){
      return message.error(`**${message.author.tag}**, no results were found for your query **${query}**!`);
    };
    ...
    msg = await message.channel.send({embeds: [pages.firstPage]});
    ...
    await pagesCollector(msg, message.author, 20000, pages)
